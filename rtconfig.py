...
if PLATFORM == "gcc":
    ...
    LFLAGS = (
        DEVICE
        + " -Wl,--gc-sections,-Map=build/target/rtthread.map,-cref,-u,Reset_Handler -T script/fsp.ld -L script/"
    )
    ...
    POST_ACTION = (
        OBJCPY + " -O ihex $TARGET build/target/rtthread.hex\n" + SIZE + " $TARGET \n"
    )
    ...
    LFLAGS = (
        DEVICE
        + " -Wl,--gc-sections,-Map=rt-thread.map,-u,Reset_Handler -lcrt0 -T script/fsp.ld -L script/"
    )
